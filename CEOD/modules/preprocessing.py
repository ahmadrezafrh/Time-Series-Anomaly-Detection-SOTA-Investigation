...
class load_data:
    ...
    def __init__(self, freq=100, rand_size=None, select_signals=None, select_action=None, save=True):
        self.freq = freq
    ...
    def train(self, path):
        ...
        train_data = self.ChangeDataScale(df_action)
        if self.rand_size:
    ...
    def ChangeDataScale(self, df_action):
        ...
        train_data[cols] = minmax_scale(train_data[cols])
        train_data['action'] = df_action['action']
        # train_data = normalize(train_data, "l2")
    ...
    def val(self, data_path):
        ...
        filepath_csv = [os.path.join(data_path, f"rec{r}_collision_20220811_rbtc_{1 / self.freq}s.csv") for r in [1]]
        filepath_meta = [os.path.join(data_path, f"rec{r}_collision_20220811_rbtc_{1 / self.freq}s.metadata") for r in
                         [1]]
        ...
        test_data = self.ChangeDataScale(test_data)
        if self.rand_size:
    ...
    def test(self, data_path):
        ...
        filepath_csv = [os.path.join(data_path, f"rec{r}_collision_20220811_rbtc_{1 / self.freq}s.csv") for r in [5]]
        filepath_meta = [os.path.join(data_path, f"rec{r}_collision_20220811_rbtc_{1 / self.freq}s.metadata") for r in
                         [ 5]]
        df_action, df, df_meta, action2int = get_df_action(filepath_csv, filepath_meta)
        df_action.index = df_action.index.map(mapper=(lambda x: pd.Timestamp(x).timestamp()))
...
def plot_data(path, freq, signals, file_name='data'):
    ...
    colors = px.colors.sample_colorscale("greys",
                                         [n / (n_colors - 1) for n in range(n_colors)])  # From continuous colormap
    ...
    for action in df_action.loc[df_interval.index].action.unique():
        df_action_interval = normalize(df_action.loc[df_interval.index])
        df_action_single_action = normalize(df_action_interval[df_action_interval['action'] == action])
