...
from our_modules.utils import check_path
...
# Modified by Omar
def get_data_fp1(dataset_normal, dataset_test, model_save_path, freq=10, rand_size=None, select_signals=None, select_action=None, save=True):
    loader = load_data(freq, rand_size, select_signals, select_action, save)
    train_data, df = loader.train(dataset_normal)
    test_data, test_label, df = loader.test(dataset_test)

    # Drop the last column which corresponds to "duration"
    # train_data = train_data[:, :51]
    # test_data = test_data[:, :51]

    test_label = np.where(test_label == 1, True, False)
