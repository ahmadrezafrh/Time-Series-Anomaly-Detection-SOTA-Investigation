...
class load_data:
    ...
    def train(self, path):
        ...
        # Omar modification to remove duration
        train_data = train_data[:, :51].astype(float)
    ...
    def test(self, data_path):
        ...
        test_data = self.scaler.transform(test_data) # Added this myself
        test_data = self.sel.transform(test_data)
        # Omar modification to remove duration
        test_data = test_data[:, :51].astype(float)
        if self.rand_size:
